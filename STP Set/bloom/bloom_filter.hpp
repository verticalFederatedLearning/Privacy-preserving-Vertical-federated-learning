...
#include "../rpc/include/jsonParser.h"
...
public:
   ...
   {
      JsonParser json;
      json["salt_count_"]=salt_count_;
      json["table_size_"]=table_size_;
      std::string arr="[]";
      JsonParser bit_table(&arr,JsonParser::ARRAY);
      for (auto now:bit_table_)
         bit_table.add(now);
      json["bit_table_"]=bit_table;
      JsonParser salt(&arr,JsonParser::ARRAY);
      for (auto now:salt_)
         salt.add(now);
      json["salt_"]=salt;
      json["projected_element_count_"]=projected_element_count_;
      json["inserted_element_count_"]=inserted_element_count_;
      return json;
   }

   bloom_filter(JsonParser json)
   {
      salt_count_=json["salt_count_"].toInt();
      table_size_=json["table_size_"].toInt();
      json["bit_table_"].foreach([&](JsonParser &paser){
         bit_table_.push_back(paser.toInt());
      });
      json["salt_"].foreach([&](JsonParser &paser){
         salt_.push_back(paser.toInt());
      });
      projected_element_count_=json["projected_element_count_"].toInt();
      inserted_element_count_=json["inserted_element_count_"].toInt();
   }
   void update(const bloom_parameters& p)
   {
      random_seed_=(p.random_seed * 0xA5A5A5A5) + 1;
      desired_false_positive_probability_=p.false_positive_probability;
   }
   ...
   {
      int result=0;
      for (auto &&bit:bit_table_)
         result+=__builtin_popcount(bit);
      return result;
   }
